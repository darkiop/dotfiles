...
![open](https://img.shields.io/badge/open-32-blue) ![done](https://img.shields.io/badge/done-37-brightgreen) ![dropped](https://img.shields.io/badge/dropped-8-lightgrey)
...
| BUG-023 | The `update-motd-network` systemd timer rewrites the network cache with the old 256-colour codes | 🐛 Bug | ✅ done [`fa3287f`](https://github.com/darkiop/dotfiles/commit/fa3287f) | follow-up to BUG-022, which only fixed the live render. `motd/systemd/update-motd-network.sh` carried its own `C_GREEN=$'\x1b[38;5;83m'` / `C_RED=$'\x1b[38;5;196m'` copy and overwrote `~/.cache/dotfiles/motd/network` every 5 minutes, so the correct colours survived only until the next timer run. Reproduced: cache mtime matched the timer's last run exactly. The colours now live in `motd/palette.sh`, sourced by the wrapper, `widgets.sh`, the timer script and the per-host iobroker widget |
| BUG-024 | `dot env`, `dot alias` and `dot modules` still print the pre-catppuccin colours | 🐛 Bug | ✅ done [`3dd447f`](https://github.com/darkiop/dotfiles/commit/3dd447f) | three colour blocks in `components/dot_help` hardcoded their own palette (`\033[38;5;83m` / `\033[38;5;196m` in `dot alias`, plain ANSI 32/33/31/36/90 in the other two), so the same green meant two different things depending on which command you ran. The catppuccin values now live in `config/catppuccin.sh` as `CAT_*` raw escapes, sourced by `config/dotfiles.config` and by `motd/palette.sh`; each assignment keeps its old value as a fallback. Follow-up in the same commit range: the generic `COLOR_*` names in `config/dotfiles.config` (used by `dot doctor`, the `dot help` frame, `install.sh` and `components/bash_defaults`) were still the 256-colour set, so they now map onto `CAT_*` too |
...
| BUG-023 | Share one MOTD status palette across render, timer and host widgets            | 🐛 Bug      | [`fa3287f`](https://github.com/darkiop/dotfiles/commit/fa3287f)                                                                                                                     | `motd/palette.sh`; the systemd timer kept restoring the old colours |
| BUG-024 | One catppuccin palette for the MOTD and the `dot` commands           | 🐛 Bug      | [`3dd447f`](https://github.com/darkiop/dotfiles/commit/3dd447f)                                                                                                                     | `config/catppuccin.sh` (`CAT_*`); `dot env`/`alias`/`modules` were still 256-colour |
